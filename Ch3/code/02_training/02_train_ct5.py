...
OUTPUT_DIR = r"C:\Users\syyda\Desktop\Chapter 3 mapping and analysis\02_Training\02_Model_Cluster5_Results"
os.makedirs(OUTPUT_DIR, exist_ok=True)

MODEL_PATH = os.path.join(OUTPUT_DIR, 'final_model_cluster5.pkl')
SHAP_PATH = os.path.join(OUTPUT_DIR, 'shap_values_cluster5.csv')
PREDS_PATH = os.path.join(OUTPUT_DIR, 'predictions_detailed_cluster5.csv')
...
# Define Predictors (Cluster + SHAP threshold 5)
...
if os.path.exists(MODEL_PATH) and os.path.exists(SHAP_PATH) and os.path.exists(PREDS_PATH):
    ...
    study = optuna.create_study(direction='minimize', storage='sqlite:///optuna.db', study_name='lgbm_opt_cluster5', load_if_exists=True)
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_bar_all_features_cluster5.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'prediction_scatter_scientific_cluster5.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_nested_pie_cluster5.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_beeswarm_all_cluster5.png'), dpi=300)
plt.close()
