...
OUTPUT_DIR = r"C:\Users\syyda\Desktop\Chapter 3 mapping and analysis\02_Training\02_Model_Cluster3_Results"
os.makedirs(OUTPUT_DIR, exist_ok=True)

MODEL_PATH = os.path.join(OUTPUT_DIR, 'final_model_cluster3.pkl')
SHAP_PATH = os.path.join(OUTPUT_DIR, 'shap_values_cluster3.csv')
PREDS_PATH = os.path.join(OUTPUT_DIR, 'predictions_detailed_cluster3.csv')
...
ABBREVIATION_MAP = {
    ...
    'Human Dev Index Upstream': r'$HDI_{UP}$'
}
...
# Define Predictors (Cluster + SHAP threshold 3)


predictors = ['Lake Volume Upstream',
'Population Upstream',
'Pasture Extent Local',
'Human Footprint Local',
'Human Footprint Upstream',
'Road Density Local',
'Potential Evap Local',
'Land Surface Runoff Local',
'Human Dev Index Local',
'Cropland Extent Local',
'Wetland Land Upstream',
'Wetland All Local',
'Terrain Slope Local',
'Elevation Local',
'Terrain Slope Upstream']
...
if os.path.exists(MODEL_PATH) and os.path.exists(SHAP_PATH) and os.path.exists(PREDS_PATH):
    ...
    study = optuna.create_study(direction='minimize', storage='sqlite:///optuna.db', study_name='lgbm_opt_cluster3', load_if_exists=True)
...
for cat, feats in CATEGORY_MAP.items():
    for f in feats:
        ...
        FEATURE_TO_CAT[f.replace('_', ' ')] = cat

CAT_COLORS = {
    'Hydrology': '#1f77b4', 'Physiography': '#7f7f7f', 'Climate': '#ff7f0e',
    'Landcover': '#2ca02c', 'Anthropogenic': '#d62728', 'Other': '#333333'
}
...
final_colors = [get_category_info(f)[0] for f in shap_importance['Feature']]
final_cats = [get_category_info(f)[1] for f in shap_importance['Feature']]

import matplotlib.patches as mpatches
unique_cats = sorted(list(set(final_cats)))
legend_handles = [mpatches.Patch(color=CAT_COLORS.get(cat, '#333333'), label=cat) for cat in unique_cats]
...
plt.yticks(name='Times New Roman', fontsize=12) 
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_bar_all_features_cluster3.png'), dpi=300)
plt.close()
...
plt.title(f"Model Performance (Cross-Validation)\n$R^2$ = {r2_cv:.3f}, RMSE = {rmse_cv:.3f}", fontname='Times New Roman', fontsize=16)
plt.legend(loc='upper left')
plt.grid(True, linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'prediction_scatter_scientific_cluster3.png'), dpi=300)
plt.close()
...
def adjust_color(c, scale):
    ...
    base = mc.to_rgb(c)
    if scale == 'SUB': 
        return tuple(max(0, x * 0.7) for x in base) 
    elif scale == 'UP': 
        return (*base, 0.5) 
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_nested_pie_cluster3.png'), dpi=300)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'shap_beeswarm_all_cluster3.png'), dpi=300)
plt.close()
